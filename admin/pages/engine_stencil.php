...
switch ( $_GET['p'] ) {
	...
	case "tambah":
		...
		$conn->query("INSERT INTO stencil VALUES ('','$dl','$wfno','$docno','$partno','$partname','$effect','$model','$issue')");
		...
		if( in_array( $ext, $ekstensi ) ) {
				...
				if ( move_uploaded_file( $_FILES['file']['tmp_name'][$i], $dir_file . $nama_file ) ) {					
					$conn->query("INSERT INTO files VALUES ('','$docno','stencil','$nama_file','./files/stencil/$nama_file')");
	...
	case "hapus" :
		...
		$id = $_GET['id'];
		$conn->query("DELETE FROM stencil WHERE stencil_docno = '$id'");
		$conn->query("DELETE FROM files WHERE file_docno = '$id'");
		$_SESSION['info'] = "Menghapus";
		header("location:admin_home.php?page=view_stencil");
